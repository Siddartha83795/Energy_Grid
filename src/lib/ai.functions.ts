...
export const analyzeEnergy = createServerFn({ method: "POST" }).handler(async () => {
  ...
  // 2. Fetch energy entries
  const entries = await db.collection("energy_entries").find({ user_id: session.userId }).toArray();
