...
export const getActionQueue = createServerFn({ method: "GET" }).handler(async () => {
  ...
  if (actions.length === 0) {
    const latestRun = await db.collection("runs")
    ...
    if (latestRun && latestRun.machines) {
      ...
      if (key) {
        ...
        const offenders = latestRun.machines.filter((m: any) => m.idle_kwh > 50 || m.utilization_pct < 70);
        
        for (const off of offenders.slice(0, 3)) {
          const savings = off.idle_cost_inr;
          
          // Groq call to generate root cause reasoning
          const sysPrompt = "You are an industrial energy root-cause analyst. Return ONLY a single sentence explanation of why a machine was left running in standby mode.";
          ...
          await db.collection("action_log").insertOne({
            ...
            action: isHighImpact ? "Power down main breaker switch and cooling pump" : "Send automatic low-power standby command",
...
export const updateActionStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await db.collection("action_log").updateOne(
...
export const getLeaderboardStats = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const machineRankings = Array.from(mStats.entries()).map(([name, val]) => {
  ...
  const shiftRankings = Object.entries(shiftStats).map(([name, val]) => {
    const active = val.total - val.idle;
    const util = val.total > 0 ? (active / val.total * 100) : 100;
    return {
      shift_name: name,
...
export const getRecStatuses = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const items = await db.collection("recommendation_status")
      .find({ user_id: userId, run_id: data.runId }).toArray();
...
export const updateRecStatus = createServerFn({ method: "POST" })
  .inputValidator((input: unknown) => {
    const o = input as { runId: string; rank: number; status: "Accepted" | "Rejected" | "Implemented" };
...
export const getSensorReadings = createServerFn({ method: "GET" }).handler(async () => {
  ...
  if (readings.length === 0) {
    const defaultSensors = [
      { machine_id: "CNC_Mill_1", temperature: 25.4, vibration_db: 32.1, status: "Likely true idle waste" },
      { machine_id: "CNC_Lathe_1", temperature: 24.2, vibration_db: 30.5, status: "Likely true idle waste" },
      { machine_id: "Robotic_Welder_1", temperature: 26.8, vibration_db: 58.4, status: "Possible stuck-on fault — investigate" },
      { machine_id: "Pick_Place_1", temperature: 22.5, vibration_db: 28.0, status: "Likely true idle waste" },
      { machine_id: "Assembly_Conveyor_1", temperature: 23.1, vibration_db: 29.5, status: "Likely true idle waste" },
      { machine_id: "Injection_Molder_2", temperature: 138.5, vibration_db: 81.2, status: "Active operation" },
      { machine_id: "Thermoformer_2", temperature: 108.2, vibration_db: 74.5, status: "Active operation" },
      { machine_id: "Packaging_Robot_2", temperature: 23.8, vibration_db: 31.0, status: "Likely true idle waste" },
      { machine_id: "Labeling_Machine_2", temperature: 21.9, vibration_db: 25.2, status: "Likely true idle waste" },
      { machine_id: "Palletizer_2", temperature: 22.8, vibration_db: 28.9, status: "Likely true idle waste" }
    ];
  ...
  return readings.map(r => {
    ...
    const power_factor = r.power_factor !== undefined ? r.power_factor : (isActive ? 0.92 + Math.random() * 0.06 : isFault ? 0.82 + Math.random() * 0.05 : 0.72 + Math.random() * 0.06);
    const harmonics_thd_pct = r.harmonics_thd_pct !== undefined ? r.harmonics_thd_pct : (isActive ? 1.5 + Math.random() * 2.0 : isFault ? 8.0 + Math.random() * 4.0 : 5.0 + Math.random() * 3.0);
    
    // Simulate condition monitoring
    const acoustic_db = r.acoustic_db !== undefined ? r.acoustic_db : (isActive ? 78.0 + Math.random() * 12.0 : isFault ? 62.0 + Math.random() * 8.0 : 35.0 + Math.random() * 8.0);
    const current_signature = r.current_signature !== undefined ? r.current_signature : (isActive ? "loaded-draw" : isIdleWaste ? "idle-draw" : "no-draw");
    
    // Simulate environmental & auxiliary
    const compressed_air_pressure_bar = r.compressed_air_pressure_bar !== undefined ? r.compressed_air_pressure_bar : (6.0 + Math.random() * 1.5);
    const compressed_air_flow_rate = r.compressed_air_flow_rate !== undefined ? r.compressed_air_flow_rate : (isActive ? 4.0 + Math.random() * 6.0 : isIdleWaste ? 0.3 + Math.random() * 0.6 : 0.0);
    const hvac_occupancy = r.hvac_occupancy !== undefined ? r.hvac_occupancy : (isActive || isFault ? true : Math.random() > 0.6);
    const lux_level = r.lux_level !== undefined ? r.lux_level : (hvac_occupancy ? 300 + Math.random() * 100 : 80 + Math.random() * 50);
    const water_flow_rate = r.water_flow_rate !== undefined ? r.water_flow_rate : (isActive ? 5.0 + Math.random() * 8.0 : 0.0);
    
    // Simulate machine state / context
    const limit_switch = r.limit_switch !== undefined ? r.limit_switch : (isActive ? "Active (Closed)" : "Inactive (Open)");
    const operator_beacon = r.operator_beacon !== undefined ? r.operator_beacon : (isActive ? "Present" : "Absent");
    ...
    const protocol = r.protocol !== undefined ? r.protocol : (protocolMap[r.machine_id] || "Modbus RTU");
    
    // Simulate actuation
    const actuator_relay = r.actuator_relay !== undefined ? r.actuator_relay : (isIdleWaste ? "ON" : isActive ? "ON" : "STANDBY");
    const vfd_speed_pct = r.vfd_speed_pct !== undefined ? r.vfd_speed_pct : (isActive ? 80 + Math.random() * 20 : isIdleWaste ? 15 + Math.random() * 10 : 0);
