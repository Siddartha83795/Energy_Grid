    ...
    <div className="p-8 max-w-6xl mx-auto space-y-6">
      ...
      <Tabs defaultValue="staff">
        <TabsList>
          ...
          <TabsTrigger value="entries">Energy entries</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="staff">
          <div className="rounded-2xl border bg-card shadow-sm">
                ...
                {staff.map((s) => (
                  <TableRow key={s.id}>
                    ...
                    <TableCell className="text-right space-x-1">
                      <Button variant="ghost" size="sm" onClick={() => toggleRole(s)}>
                        Make {s.role === "admin" ? "staff" : "admin"}
                      </Button>
            ...
            <p className="p-3 text-xs text-muted-foreground border-t">
              To add a new staff member, ask them to sign up at <code>/auth</code>; they'll appear
              here.
            </p>
          </div>
        </TabsContent>
                      ...
                      <TableCell>
                        {editing ? (
                          <Input
                            ...
                            onChange={(ev) => setDraft({ ...draft, machine_name: ev.target.value })}
                          />
                        ) : (
                          e.machine_name
                        )}
                      ...
                      <TableCell>
                        {editing ? (
                          <Input
                            ...
                            type="number"
                            value={draft.kwh ?? ""}
                            onChange={(ev) => setDraft({ ...draft, kwh: Number(ev.target.value) })}
                          />
                        ) : (
                          (e.kwh ?? "—")
                        )}
                      ...
                      <TableCell>
                        {editing ? (
                          <Input
                            className="h-8 w-20"
                            type="number"
                            value={draft.total_kwh ?? ""}
                            onChange={(ev) =>
                              setDraft({ ...draft, total_kwh: Number(ev.target.value) })
                            }
                          />
                        ) : (
                          (e.total_kwh ?? "—")
                      ...
                      <TableCell>
                        {editing ? (
                          <Input
                            className="h-8 w-20"
                            type="number"
                            value={draft.idle_kwh ?? ""}
                            onChange={(ev) =>
                              setDraft({ ...draft, idle_kwh: Number(ev.target.value) })
                            }
                          />
                        ) : (
                          (e.idle_kwh ?? "—")
