    ...
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      ...
      {actions && actions.length > 0 && (
        <div className="space-y-4">
          {actions.map((act) => (
            <div key={act.id} className="rounded-2xl border bg-card p-5 space-y-3 shadow-sm hover:border-primary/20 transition-all">
              ...
              <div className="text-sm">
                <span className="font-medium text-muted-foreground">Recommended Action:</span> {act.action}
              </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t flex-wrap gap-2">
                ...
                {act.status === "approved" && (
                  <div className="text-xs text-green-600 font-medium flex items-center gap-1">
                    <CheckCircle2 className="h-3.5 w-3.5" /> Action approved and simulated control signal sent
                  </div>
                )}
                {act.status === "rejected" && (
